---
title: "Penguins dashboard example"
# author: Kasia Kedzierska
output: 
  flexdashboard::flex_dashboard:
    logo: figs/penguins_small.png
    theme:
      bootswatch: minty
    orientation: rows
...
<img src="figs/penguins.png" width="50%" alt="The Palmer Archipelago penguins. Artwork by @allison_horst.">

The Palmer Archipelago penguins. Artwork by @allison_horst.

### What do we measure when looking at culmen/bill?

<img src="figs/culmen_depth.png" />
...
# add histogram of flipper lengths here 

penguins %>%
  rename(pretty_penguin_axis) %>%
...
```{r}
# add summary of each value per group 
penguins %>% 
  group_by(species) %>% 
  summarize(across(where(is.numeric), mean, na.rm = TRUE)) %>% 
  rename(c(pretty_penguin_axis, "Species" = "species", "Year" = "year")) %>%
  datatable(rownames = FALSE) %>%
  formatRound(names(pretty_penguin_axis), 2) %>%
  formatRound("Year", 0)
```
